...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const firstPoint = (x2 - x1) ** 2;
  const secondPoint = (y2 - y1) ** 2;
  return Math.sqrt(firstPoint + secondPoint);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const firstNumber = x1 * x2 + y1 * y2;
  const secondNumber = Math.sqrt(x1 ** 2 + y1 ** 2);
  const thirdNumber = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(firstNumber / (secondNumber * thirdNumber));
}
...
function roundToPowerOfTen(num, pow) {
  const newNumber = 10 ** pow;
  return Math.round(num / newNumber) * newNumber;
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    resultNumber = previousNumber + nextNumber;
    previousNumber = nextNumber;
    nextNumber = resultNumber;
  }
  return nextNumber;
}
...
function getSumOfDigits(num) {
  const newArray = Array.from(String(num), Number);
